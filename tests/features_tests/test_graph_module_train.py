...
class MutatingOptimizer(Optimizer):
    ...
    def _step(self, *args, **kwargs):
        ...
        for p in self.parameters:
            if p.name == "synth_prompt" and self.calls == 1:
                updates[p] = "CRISPR optimized :: {query} :: {plan}"
        return updates
...
def build_adapter():
    ...
    def synth_node(state):
        ...
        return {"final_answer": synth_prompt.data.replace("{query}", str(query)).replace("{plan}", str(plan))}
...
def test_graphmodule_is_train_compatible():
    adapter = build_adapter()
    model = adapter.as_module()
    guide = KeywordGuide()
    optimizer = MutatingOptimizer(model.parameters())
    algo = train(
        model=model,
        train_dataset={"inputs": ["gene editing"], "infos": ["CRISPR"]},
